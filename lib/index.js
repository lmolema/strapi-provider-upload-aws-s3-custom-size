...
const resize = async (buffer, width, quality) =>
  sharp(buffer)
    .resize({ width: parseInt(width) })
    .jpeg({ quality: parseInt(quality) })
    .toBuffer()
    .then(data =>
      decodeBase64Image(`data:image/jpeg;base64,${data.toString('base64')}`)
    );

const S3upload = S3 => (file, width, quality) =>
  new Promise(async (resolve, reject) => {
    const { mime, hash, path: filePath, buffer, ext } = file;
    const path = filePath ? `${filePath}/` : '';
    const resizedBuffer = await resize(buffer, width, quality);
    const fileName =
      (file.related && file.related.length > 0 && file.related[0].refId) ||
      hash;
    S3.upload(
      {
    ...
    return {
      upload: file => {
        return new Promise((resolve, reject) => {
          ...
          S3upload(S3)(file, config.width, config.quality)
            .then(url => {
              file.url = url;
              resolve();
            })
